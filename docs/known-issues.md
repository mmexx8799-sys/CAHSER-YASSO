...
| D-O8 | نطاق الرفض الفوري (طبقة الخدمة) | **(أ) assertOnline() في أول كل دالة كتابة في `services/api.ts`** — بدون `clientErrors` (fire-and-forget) وبدون Auth — تشمل 21 دالة كتابة (6 + 15 — انظر التفاصيل) + تعطيل أزرار AC-03 فقط | Owner | 2026-09-21 |
