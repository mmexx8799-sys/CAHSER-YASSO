    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border-l-4 ${isOpen ? 'border-green-500' : 'border-red-500'}`}>
      <div className="p-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className={`px-2 py-1 text-xs font-semibold rounded-full ${isOpen ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400'}`}>
            {isOpen ? 'مفتوح' : 'مغلق'}
          </span>
        </div>

        <div className="text-center my-4">
          <p className="text-sm text-gray-500 dark:text-gray-400">صافي النقدية بالدرج</p>
          <p className="text-3xl font-extrabold text-blue-600 dark:text-blue-400 my-1">{netCashInDrawer.toFixed(2)} ج.م</p>
        </div>

        <div className="flex justify-around text-center text-sm border-t border-gray-200 dark:border-gray-700 pt-3">
          <div>
            <p className="text-gray-500 dark:text-gray-400">إجمالي المبيعات</p>
            <p className="font-bold text-green-600 dark:text-green-400">{totalSales.toFixed(2)} ج.م</p>
          </div>
          <div>
            <p className="text-gray-500 dark:text-gray-400">إجمالي المرتجعات</p>
            <p className="font-bold text-red-600 dark:text-red-400">{totalReturns.toFixed(2)} ج.م</p>
          </div>
        </div>
    ...
    <div className="p-4">
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <p className="text-center text-gray-500 mt-8">لا يوجد أرشيف لعرضه.</p>
        ) : (
