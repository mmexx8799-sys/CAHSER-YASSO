...
import type { Product, CartItem, DailyArchive, Category } from '../types';
import { getProductsPaginated, getOpenDailyArchive, processReturn } from '../services/api';
    ...
    >
        ...
        <div className="mt-auto pt-2 border-t border-gray-100 w-full flex justify-between items-end">
            ...
            <p className="text-primary-700 font-bold text-xl">
                {product.price.toFixed(2)} <span className="text-[10px] font-normal text-gray-500">ج.م</span>
            </p>
        </div>
    </div>
);
...
const ProductGrid = memo(({
    ...
    return (
        <>
            ...
            {!hasMore && products.length > 0 && <div className="text-center p-4 text-gray-500">لا يوجد المزيد من المنتجات.</div>}
            {!isLoading && products.length === 0 && <div className="text-center p-10 text-gray-500">لم يتم العثور على منتجات.</div>}
        </>
    );
});
...
const ReturnCartModal: React.FC<{
    ...
    const handleProcessReturn = async () => {
        ...
        try {
            await processReturn(returnCart, dailyArchive.id);
            clearCart();
            ...
            <div className="bg-white dark:bg-gray-800 rounded-t-lg shadow-xl p-4 w-full max-w-lg h-3/4 flex flex-col">
                <div className="flex justify-between items-center mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">
                    ...
                    <div className='flex items-center gap-4'>
                        ...
                        <button onClick={() => !isProcessing && setCartModalOpen(false)} disabled={isProcessing}><X /></button>
                    </div>
                </div>
                {returnCart.length === 0 ? (
                ...
                <div className="p-2 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 mt-auto">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-lg font-semibold">إجمالي المرتجع</span>
                        <span className="text-2xl font-bold text-red-600">{total.toFixed(2)} ج.م</span>
                    </div>
                    <button
            ...
            {returnCart.length > 0 && (
                ...
                >
                    <div className="flex items-center space-x-2 space-x-reverse">
                        <Undo2 />
                        <span className="bg-white text-red-600 rounded-full w-6 h-6 flex items-center justify-center font-bold text-sm">{returnCart.length}</span>
                    </div>
                    <span className="font-bold text-lg">سلة المرتجعات</span>
                    <span className="font-extrabold text-lg">{total.toFixed(2)} ج.م</span>
                </button>
            )}
